...
function Navbar({ active = "about", onNavigate }) {
  ...
  const items = [
  { id: "about", label: "關於我" },
  { id: "projects", label: "精選專案" },
  { id: "skills", label: "技術與證照" },
  { id: "life", label: "生活角落" },
    ...
    <nav className={`portfolio-nav ${scrolled ? "is-scrolled" : ""}`}>
      <div className="portfolio-nav__inner">
        <a className="portfolio-brand" href="#" onClick={(e) => {e.preventDefault();onNavigate?.("top");}}>
          ...
          <span className="portfolio-brand__wd">Sin-Cen個人介紹</span>
        </a>
        ...
        <button className="portfolio-nav__cta" onClick={() => onNavigate?.("contact")} style={{ color: "rgb(237, 149, 185)" }}>
          聊聊
        </button>
      </div>
    </nav>);
