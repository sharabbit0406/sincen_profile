...
(() => {
  ...
  // Cert photos / portraits in this kit are viewed in a fullscreen lightbox,
  // so we keep more pixels than the deck-thumbnail default.
  const MAX_DIM = 2400;
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      const cap = Math.min(MAX_DIM, Math.max(1, Math.round(targetW * 5)) || MAX_DIM);
