...
function FloatDock({ onNavigate, onAboutOpen, onChat }) {
  ...
  const Icon = {
    ...
    bot: <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="10" rx="2"/><path d="M9 11V9a3 3 0 0 1 6 0v2"/><circle cx="9" cy="16" r="1" fill="currentColor" stroke="none"/><circle cx="15" cy="16" r="1" fill="currentColor" stroke="none"/><path d="M12 3v3"/></svg>,
  ...
  const goChat = () => { onChat?.(); setOpen(false); };

  const actions = [
    showScrollTop && { key: "top", label: "回到頂部", icon: Icon.arrowUp, onClick: goTop, tone: "sky" },
    { key: "chat-ai", label: "ＡＩ聊聊", icon: Icon.bot, onClick: goChat, tone: "sky" },
